const icons = {
  ...
  logout: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 17 15 12 10 7"/><path d="M15 12H3"/><path d="M21 19V5a2 2 0 0 0-2-2h-5"/><path d="M14 21h5a2 2 0 0 0 2-2"/></svg>',
...
function wireLanding() {
  ...
  if (error && params.get("error") === "1") {
    error.hidden = false;
  }

  if (error && params.get("error") === "config") {
    error.textContent = "Password access is not configured yet. Add BACKND_PASSWORD in Vercel.";
    error.hidden = false;
  }
